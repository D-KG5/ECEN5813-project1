...
void dec2bin1s(int16_t dec, uint16_t operand){ 
    int i; 
    // iterate over decimal of bit size (operand) based on its negated absolute value
    for (i = 1 << operand; i > 0; i = i / 2){
        (~(abs(dec)) & i) ? fprintf(fptr, "1") : fprintf(fptr, "0"); 
    }
} 

// function to print 2's complement binary
void dec2bin2s(int16_t dec, uint16_t operand){ 
    int i; 
...
int main(void){
	...
	for(int i = 0; i < 11; i++){
		// check if radix and operands are valid, else print Error to file
